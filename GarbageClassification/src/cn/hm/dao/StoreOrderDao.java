...
public interface StoreOrderDao {
	...
	//创建订单
	boolean createStoreOrder(StoreOrder order);
	//获取订单
	List<StoreOrder> getStoreOrder(int uid,int ostate);
	//创建订单条目
	boolean createStoreOrderItem(List<StoreOrderItem> list);
	//获取订单条目
	List<StoreOrderItem> getStoreOrderItem(String oid);
	//获取订单
	StoreOrder getOrder(String oid);
	//删除订单条目
	boolean deleteOrderItem(String oid) throws SQLException;
	//删除订单
